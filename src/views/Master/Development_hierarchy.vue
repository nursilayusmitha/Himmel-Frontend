...
const pageTitle = ref('Development - Hierarchy')
const pageList = ref(['Development', 'Hierarchy'])
const dataHeader = ref([
  {name: 'No', class: 'min-w-[30px] py-2 px-4'}, // Kolom nomor
  {name: 'Nama Hirarki', class: 'min-w-[150px] py-2 px-4'},
  {name: 'Kode Hirarki', class: 'min-w-[130px] py-2 px-4'},
  {name: 'Grup Hirarki', class: 'min-w-[150px] py-2 px-4'},
  {name: 'Deskripsi', class: 'min-w-[100px] py-2 px-4'},
]);
let dataTable = ref([])
...
onMounted(async () => {
  const pageData = await indexStore.getUtilHierarchy();
  dataTable.value = pageData.utilHierarchy.map(item => ({
    ...item,
  }));
});
            ...
            <tbody>
              <tr v-for="(item, index) in paginatedData" :key="index" class="border">
                ...
                <td class="py-1 px-4 border">
                  <p class="text-black dark:text-white text-xs">{{ item.hierarchyName }}</p>
                </td>
                <td class="py-1 px-4 border">
                  <p class="text-black dark:text-white text-xs text-center">{{ item.hierarchyCode }}</p>
                </td>
                <td class="py-1 px-4 border">
                  <p class="text-xs text-black dark:text-white">{{ item.hierarchyGroup }}</p>
                </td>
                <td class="py-1 px-4 text-center border">
                  <h5 class="font-medium text-black text-xs dark:text-white text-left">{{ item.hierarchyDesc }}</h5>
                </td>
              </tr>
            </tbody>
          </table>
