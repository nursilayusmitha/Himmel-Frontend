...
import { getUserByRequest } from '@/stores/functionAPI'
import Swal from 'sweetalert2'
import { mdiTrashCanOutline, mdiEyeOutline, mdiAccountCheck , mdiMagnify, mdiRefresh, mdiPlusCircleOutline, mdiAccountCancel  } from '@mdi/js';

const pageTitle = ref('Master - User Internal Request')
const pageList = ref (['Master', 'User Internal Request'])
...
const confirmDelete = (id) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      try {
        const response = await fetch(`http://localhost:4000/api/userInternal/delete/${id}`, {
          method: 'DELETE',
        });
        if (response.ok) {
...
const confirmAccept = (id) => {
  ...
  Swal.fire({
    title: 'Apakah anda yakin?',
    text: "Anda akan mengaktifkan akun user dan menerima user sebagai bagian dari Internal Himmel.",
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33',
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`http://localhost:4000/api/userInternal/accept/${id}`, {
          method: 'DELETE',
        });
        if (response.ok) {
          // Tampilkan custom success alert
          successBlock('Mengaktifkan User Berhasil!', '', 'center');
          
          // Update dataTable to remove the deleted item
          dataTable.value = dataTable.value.filter(item => item._id !== id);
          
        } else {
          Swal.fire('Failed!', 'Gagal menerima user.', 'error');
        }
      } catch (error) {
        Swal.fire('Error!', 'Terjadi kesalahan saat menerima user.', 'error');
      }
    }
  });
};
...
const successBlock = async (title, text, position) => { 
  Swal.fire({
    position: position || "center",
    title: title || 'Mengaktifkan User Berhasil!', 
    text: text || '',
    timer: 1500,
    icon: 'success',
    showConfirmButton: false,
...
<template>
  ...
  @click="confirmAccept(item._id)"
>
  <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path fill-rule="evenodd" clip-rule="evenodd" :d="mdiAccountCheck " fill="#" />
  </svg>
</button>
