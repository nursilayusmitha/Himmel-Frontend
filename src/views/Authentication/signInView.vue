...
import { userEksternal_Login } from "@/stores/functionAPI";
...
const logCredentials = async () => {
  ...
  try {
    const loginData = await userEksternal_Login(data);
  ...
  <DefaultAuthCard subtitle="Start now" title="Sign In to Himmel">
    <form>
      ...
      <div class="mt-6 text-center">
        <p class="font-medium">
            Lupa Password?
            <router-link to="/forget-password" class="text-gray-400 cursor-default">Pulihkan password anda</router-link>
        </p>
        <p class="font-medium text-black dark:text-white mt-1">
            Belum punya akun?
        <router-link to="/sign-up" class="text-[#009ee5] hover:underline">Yuk, Daftar sekarang!</router-link>
        </p>
      </div>
    </form>
  </DefaultAuthCard>
