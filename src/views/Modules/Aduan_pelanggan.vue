...
const pageTitle = ref('Informasi - Aduan Pelanggan')
const pageList = ref(['Informasi', 'Aduan Pelanggan'])
...
const currentStatusFilter = ref('active') // active = prioritize + problem
...
const fetchAduan = async (status = 'active') => {
  try {
    let result = []

    if (status === 'active') {
      const prioritize = await axios.get('http://localhost:4000/api/Modul_aduan/status/Prioritize')
      const problem = await axios.get('http://localhost:4000/api/Modul_aduan/status/Problem')

      result = [...prioritize.data, ...problem.data]
      // Urutkan prioritize dulu
      result.sort((a, b) => (a.status === 'Prioritize' ? -1 : 1))
    } else {
      const { data } = await axios.get(`http://localhost:4000/api/Modul_aduan/status/${status}`)
      result = data
    }
...
const filterByStatus = async (status) => {
  ...
  if (status === 'active') {
    await fetchAduan('active')
  } else {
    await fetchAduan(status)
  }
}
...
onMounted(async () => {
  await filterByStatus('active') // default: prioritize + problem
  setInterval(() => filterByStatus(currentStatusFilter.value), 1000)
})
...
<template>
  ...
  <div v-if="!showChatModal"
    ...
    <div class="flex space-x-2 mb-3 -mt-3 justify-end">
          <!-- Tombol Prioritize -->
<button class="bg-black text-white p-2 rounded-md" @click="filterByStatus('Prioritize')">
...
<!-- Tombol Problem -->
<button class="bg-black text-white p-2 rounded-md" @click="filterByStatus('Problem')">
  <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24">
    <path :d="mdiAccountAlert" fill="#"/>
  </svg>
</button>

<!-- Tombol Solved -->
<button class="bg-black text-white p-2 rounded-md" @click="filterByStatus('Solved')">
  <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24">
    <path :d="mdiAccountCheck" fill="#"/>
  </svg>
</button>
