...
import { getPesawatAll } from '@/stores/functionAPI'
...
const pageTitle = ref('Modul - Pesawat')
const pageList = ref (['Modul', 'Pesawat'])
...
onMounted( async () => {
  const response = await getPesawatAll();
  if (response.pesawat) {
    dataTable.value = response.pesawat;
  } else {
    Swal.fire({
      title: 'Error!',
      text: 'Failed to fetch pesawat data',
      icon: 'error',
  ...
  >
        ...
        <div class="left-data">
            ...
            <div class="right-data flex items-center flex-row-reverse">
              <!-- Add Button -->
              <router-link to="/modules/modul-pesawat/add" class="px-1">
              <svg 
    ...
    {{ item.Status_operasional === 'enable' ? 'Aktif' : 'Tidak Aktif' }}
            ...
            <td class="py-1 px-4">
              <div class="flex items-center space-x-3.5 d-flex justify-center">              
                ...
                <router-link :class="item.Tr_teknis_status === 'closed' ? 'cursor-default opacity-50' : 'hover:text-primary'" class="" :to="'/modules/modul-pesawat/detail/'+item._id">
                <svg
                  class="fill-current"
                  width="24"
                  height="24"
              ...
              <router-link :class="item.Tr_teknis_status === 'closed' ? 'cursor-default opacity-50' : 'hover:text-primary'" class="" :to="'/modules/modul-pesawat/edit/'+item._id">
                <svg
                  class="fill-current"
                  width="24"
                  height="24"
